...
public class Paytolls {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Graph<Integer> graph = new Graph<>(n_nodes);
        for (int i = 0; i < n_nodes; i++) {
            graph.adjList[i].setInfo(i);
        }


        String skopjeBerlin[] = br.readLine().split(" ");
        int skopjeId = Integer.parseInt(skopjeBerlin[0]);
        int berlinId = Integer.parseInt(skopjeBerlin[1]);
        ...
        for (int i=0; i<edges; i++){
            ...
            graph.addEdge(city1-1, city2-1, weight);
            graph.addEdge(city2-1, city1-1, weight);

        }

        float distane[] = graph.dijkstra(skopjeId-1);

//        System.out.println(Arrays.toString(distane));
        System.out.println((int)distane[berlinId-1]);
